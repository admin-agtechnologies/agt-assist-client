...
interface ToastContextType { toast: { success: (m: string) => void; error: (m: string) => void; info: (m: string) => void; warning: (m: string) => void; }; }
...
export function ToastProvider({ children }: { children: ReactNode }) {
  ...
  const toast = { success: (m: string) => add(m, "success"), error: (m: string) => add(m, "error"), info: (m: string) => add(m, "info"), warning: (m: string) => add(m, "warning") };
    ...
    <ToastContext.Provider value={{ toast }}>
      ...
      {typeof window !== "undefined" && createPortal(
        <div className="fixed bottom-6 right-6 z-[99999] flex flex-col gap-2 w-80">
          {toasts.map(t => (
            <div key={t.id} className={`${colors[t.type]} rounded-xl px-4 py-3 shadow-lg flex items-start gap-3 animate-slide-up border border-[var(--border)]`}>
