...
const STATUS_COLOR: Record<AppointmentStatus, StatusColor> = { confirmed: "green", pending: "amber", done: "blue", cancelled: "slate" };
...
export default function PmeAppointmentsPage() {
  ...
  const handleDelete = async () => {
    if (!deleteId) return;
    setIsDeleting(true);
    try { await appointmentsRepository.delete(deleteId); toast.success(t.deleteSuccess); setDeleteId(null); fetchData(); }
    ...
    <>
      <div className="space-y-6 animate-fade-in">
        ...
        <div className={cn("card overflow-hidden transition-opacity", isPending && "opacity-50 pointer-events-none")}>
          {items.length === 0 ? <EmptyState message={t.noData} icon="📅" /> : (
            <>
              <div className="hidden md:grid grid-cols-5 px-6 py-3 bg-[var(--bg)] border-b border-[var(--border)] text-[10px] font-black uppercase tracking-widest text-[var(--text-muted)]">
                <span>{t.table.client}</span><span>{t.table.service}</span><span>{t.table.date}</span><span>{t.table.channel}</span><span>{t.table.status}</span>
              </div>
              {items.map(apt => (
                <div key={apt.id} className="grid grid-cols-1 md:grid-cols-5 px-6 py-4 border-b border-[var(--border)] last:border-0 hover:bg-[var(--bg)] transition-colors items-center gap-2">
                  ...
                  <Badge variant={apt.channel === "whatsapp" ? "green" : "violet"}>{t.channels[apt.channel] ?? apt.channel}</Badge>
                  <div className="flex items-center justify-between md:justify-start gap-3">
                    <Badge variant={STATUS_COLOR[apt.status]}>{t.statuses[apt.status]}</Badge>
                    <div className="flex gap-2">
                      <button onClick={() => { setEditingId(apt.id); setModalOpen(true); }} className="p-1.5 rounded-lg hover:bg-[var(--bg)] text-[var(--text-muted)] transition-colors">✏️</button>
                      <button onClick={() => setDeleteId(apt.id)} className="p-1.5 rounded-lg hover:bg-red-50 dark:hover:bg-red-900/20 text-red-400 transition-colors">🗑️</button>
                    </div>
                  </div>
                </div>
              ))}
      ...
      <AppointmentModal isOpen={modalOpen} itemId={editingId} tenantId={user?.tenant_id ?? ""}
        onClose={() => setModalOpen(false)} onSave={fetchData} t={t} toast={toast} />
...
function AppointmentModal({ isOpen, itemId, tenantId, onClose, onSave, t, toast }: { isOpen: boolean; itemId: string | null; tenantId: string; onClose: () => void; onSave: () => void; t: any; toast: any }) {
  const isEdit = !!itemId;
  ...
  useEffect(() => {
    ...
    if (itemId) {
      setLoading(true);
      appointmentsRepository.getById(itemId).then(d => setForm({ service_id: d.service_id, client_name: d.client_name, client_phone: d.client_phone, scheduled_at: d.scheduled_at.slice(0, 16), status: d.status, notes: d.notes })).finally(() => setLoading(false));
    } else setForm(DEF);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      else await appointmentsRepository.create({ ...payload, tenant_id: tenantId } as any);
          ...
          {loading ? <div className="flex justify-center py-8"><Spinner className="w-6 h-6 border-[#25D366] border-t-transparent" /></div> : <>
            ...
            <div><label className="label-base">{tf.status}</label>
              <select className="input-base" value={form.status} onChange={e => setForm({ ...form, status: e.target.value as AppointmentStatus })}>
                {(["pending","confirmed","done","cancelled"] as AppointmentStatus[]).map(s => <option key={s} value={s}>{t.statuses[s]}</option>)}
              </select>
            </div>
